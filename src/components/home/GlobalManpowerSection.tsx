...
<ScrollReveal>
  <div className="text-center mb-16">
    ...
    {/* Main Description */}
    <p className="text-muted-foreground max-w-3xl mx-auto mb-4 whitespace-nowrap overflow-hidden text-ellipsis">
  {t("home.manpower.description")}
    ...
    <p className="text-muted-foreground max-w-3xl mx-auto">
      {t("home.manpower.additionalDescription")}
    </p>
  </div>
</ScrollReveal>
...
<ScrollReveal delay={300}>
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6">
    {manpowerCountries.map((country) => (
      <div
        key={country.key}
        className="group relative bg-primary-foreground/5 backdrop-blur-sm border border-primary/10 rounded-2xl p-4 sm:p-5 text-center transition-all duration-300 hover:bg-primary-foreground/10 hover:-translate-y-1 sm:hover:-translate-y-2"
      >
        {/* Flag */}
        <div className="w-12 h-12 mx-auto rounded-full flex items-center justify-center mb-3 sm:mb-4 shadow-glow overflow-hidden bg-transparent group-hover:scale-105 transition-transform">
          <img
        ...
        {/* Gender Indicator */}
        <p
  className="text-[10px] mt-1 sm:text-xs"
  style={{
    color: "#FF6B6B", // Light Red
  }}
>
          {country.key === "pakistan"
            ? t("home.gender.male")
            : t("home.gender.both")}
        </p>
      </div>
