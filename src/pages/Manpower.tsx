...
/** Icon mapping (icons stay in TS, text stays in JSON) */
const CATEGORY_ICONS: Record<string, any> = {
  ...
  operators: Cog,
  safety: Shield,
  mechanicalHelpers: Zap,
  electricalHelpers: Droplets,
  constructionLabor: Building,
};

const Manpower = () => {
  const { t } = useLanguage();

  /** Read categories from en.json */
  const categories = useMemo(() => {
    const data = t("manpower.categories");
    if (!data) return [];
                  ...
                  <ScrollReveal key={activeCategory.id}>
                    <GlassCard className="p-8">
                      ...
                      <h3 className="font-display font-semibold text-lg mb-4">
                        {t("manpower.availablePositions")}
                      </h3>

                      <div className="grid sm:grid-cols-2 gap-3">
                        {activeCategory.workers.map((worker: string) => (
                          <div
                            key={worker}
                      ...
                      <div className="mt-8 pt-6 border-t">
                        <h4 className="font-display font-semibold mb-3">
                          {t("manpower.whyChoose")}
                        </h4>
                        <ul className="grid sm:grid-cols-2 gap-2 text-sm text-muted-foreground">
  <li className="flex items-center gap-2">
    <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
    {t("manpower.benefits.verified")}
  </li>

  <li className="flex items-center gap-2">
    <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
    {t("manpower.benefits.training")}
  </li>

  <li className="flex items-center gap-2">
    <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
    {t("manpower.benefits.certifications")}
  </li>

  <li className="flex items-center gap-2">
    <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
    {t("manpower.benefits.deployment")}
  </li>
</ul>
